...
describe ::Services::Ecf::PaymentCalculation do
  ...
  it "returns BigDecimal for all money outputs" do
    ...
    expect(result.dig(:output, :output_payments, :per_participant)).to be_a(BigDecimal)
    if result[:output][:output_payments]
      result.dig(:output, :output_payments, :output_payment_schedule).each do |_key, value|
